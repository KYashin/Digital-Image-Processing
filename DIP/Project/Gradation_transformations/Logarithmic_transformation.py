...
def logarithm(path_to_image, base):
    ...
    c = 255 / np.log(1 + np.max(img))  # Коэффициент нормализации
    img_1 = c * (np.log(1 + img.astype(np.float32)) / np.log(base))
...
logarithm(r"D:\pythonProject\DIP\Project\Images\i.webp", 2)